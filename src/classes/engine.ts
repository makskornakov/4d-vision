// import { Camera, OrthographicProjection, PerspectiveProjection, Viewport } from './3dprojection';
...
export function tickSquareDraw(
  ctx: CanvasRenderingContext2D,
  settings: { square: { size: number }; globalScale: number },
) {
  ...
  const updatedSize = settings.square.size + settings.globalScale;
  ctx.beginPath();
  ctx.rect(
